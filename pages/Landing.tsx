...
const IntroAnimation = ({ onFinish }: { onFinish: () => void }) => {
  ...
  useEffect(() => {
    // Stage 1: Leaf appears (started at 0, trigger shortly after mount)
    const t1 = setTimeout(() => setStage(1), 100);
    // Stage 2: Bg turns white, icon turns green, text slides out
  ...
  return (
    <div 
      ...
      ${stage >= 2 ? 'bg-gray-50' : 'bg-[#72b63b]'}
      ${stage === 3 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
    >
      <div className="relative flex items-center justify-center">
        {/* Animated Leaf */}
        <div className={`transition-all duration-1000 ease-out transform
          ${stage === 0 ? 'scale-0 opacity-0 rotate-[-45deg]' : ''}
          ${stage === 1 ? 'scale-150 opacity-100 rotate-0' : ''}
          ${stage >= 2 ? 'scale-100 rotate-0' : ''}
        `}>
          <Leaf 
            size={stage >= 2 ? 50 : 100} 
            className={`transition-colors duration-700 ${stage >= 2 ? 'text-[#72b63b] fill-[#72b63b]' : 'text-white fill-white'}`}
          />
        </div>

        {/* Text Reveal */}
        <div className={`
          flex items-center overflow-hidden transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)]
          ${stage >= 2 ? 'w-[280px] opacity-100 ml-4' : 'w-0 opacity-0 ml-0'}
        `}>
          <div className="flex items-center select-none whitespace-nowrap">
             <span className="text-[#1e5336] font-bold text-5xl tracking-tighter">ECO</span>
             <span className="text-[#72b63b] font-bold text-5xl tracking-tighter">TRACE</span>
          </div>
        </div>
      </div>
    </div>
